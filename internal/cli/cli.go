...
// offers.
//
// Everything ECK-Tunnel does from a terminal has until now gone through
...
const usage = `eck — non-interactive commands

  eck tunnel list [--json]         every tunnel and its state
  eck tunnel status <name> [--json]  one tunnel: state, peer, and what it has carried
  eck check -c <file>              validate a config without starting it
  eck version [--json]

Run eck with no arguments for the interactive menu.
...
func tunnelStatus(name string, asJSON bool) Result {
	...
	// The exit code carries the answer as well as the output, so `eck
...
func runCheck(args []string) Result {
	...
	for i := 0; i < len(rest); i++ {
		...
		if path == "" {
			path = rest[i] // `eck check file.toml` as well as `-c file.toml`
			continue
		}
		return fail(CodeUsage, "check takes one config file\n")
	}
	if path == "" {
		return fail(CodeUsage, "check needs a config file: eck check -c /etc/eck/x.toml\n")
	}
