// Package schedule manages recurring eck jobs via the system crontab
...
// failure of `crontab -l` turned installing a eck job into deleting every
...
// SetAutoRefresh schedules `eck --restart-all` every `hours` hours.
// hours == 0 disables it.
