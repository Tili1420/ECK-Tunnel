...
// apart, and a running binary that asked for "eck_linux_arm.tar.gz" would
...
func TestAnARMBuildAsksForItsOwnVariant(t *testing.T) {
	...
	if !strings.HasPrefix(AssetName(), "eck_linux_"+runtime.GOARCH) {
...
func TestEveryArchitectureBuiltIsAlsoPublished(t *testing.T) {
	...
	if !strings.Contains(src, "github.com/eck-tunnel/eck/internal/app.GOARM=$$v") {
		t.Error("the ARM builds are not stamped with their variant, so each would " +
			"ask for eck_linux_arm.tar.gz, which no release publishes")
	}
	// Both loops name the archive the same way the binary will.
	if !strings.Contains(src, "release/eck_linux_$$a.tar.gz") {
...
func TestTheWorkflowPublishesEverythingTheBuildProduces(t *testing.T) {
	...
	src := string(wf)
	if !strings.Contains(src, "release/eck_linux_*.tar.gz") {
