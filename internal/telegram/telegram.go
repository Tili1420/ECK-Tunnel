...
import (
	...
	"github.com/eck-tunnel/eck/internal/geo"
...
func helpText(lang string) string {
	var out strings.Builder
	out.WriteString(b("🔗 ECK-Tunnel") + " " + esc(app.Version) + "\n\n")
...
// the `eck --telegram-report` cron job.
...
func SendTest(c Config) error {
	const msg = "✅ ECK-Tunnel is connected. You will receive status reports here."
...
func sendBackup(c Config) error {
	...
	name := fmt.Sprintf("eck-backup-%s.tar.gz", time.Now().Format("2006-01-02-1504"))
