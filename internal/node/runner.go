...
// ECK-Tunnel on it is missing or too old. Both are fixed the same way, by
// installing over the same SSH connection, so both carry this.
var ErrNeedsInstall = errors.New("eck must be installed on that server")
...
func (r *SSHRunner) exec(ctx context.Context, name string, t SSHTarget, req Request) (Response, error) {
	...
	if err != nil {
		...
		stdout, err = runOver(c, cmd, nil)
		if err != nil {
			return Response{}, eckMissing(err)
		}
	}

	line := strings.TrimSpace(string(stdout))
	if line == "" {
		return Response{}, fmt.Errorf("%s answered nothing — is ECK-Tunnel installed there?", name)
	}
...
// eckMissing turns "this server cannot answer the panel" into the thing
// the operator has to do about it, which is the same thing in both of the ways
// it happens.
//
// A server that has never had ECK-Tunnel fails in the shell: "command not found".
// A server that has an older ECK-Tunnel fails inside the binary — it runs, does
...
// into the add form. It told the operator to go and run `eck node setup`,
// which is exactly the flow that was removed.
//
// Both mean the same thing to the panel — the ECK-Tunnel over there is not one
...
func eckMissing(err error) error {
	msg := err.Error()
	switch {
	case strings.Contains(msg, "not found"), strings.Contains(msg, "No such file"):
		return errNotInstalled("ECK-Tunnel is not installed on that server")
	case outdatedEck(msg):
		return errNotInstalled("the ECK-Tunnel on that server is too old to be managed " +
			"from this panel")
	}
	return err
}
...
// outdatedEck recognises a binary that ran and did not understand.
//
...
// an unknown subcommand is refused by name, and that the whole of `eck
// node`'s help follows it.
func outdatedEck(msg string) bool {
	...
	// happens to contain the word "eck" is not read as this.
	return strings.Contains(msg, "eck node") &&
...
// Install puts ECK-Tunnel on a server that does not have it, over the same SSH.
//
...
func (r *SSHRunner) Install(name string) (string, error) {
	...
	if err != nil {
		return string(out), fmt.Errorf("installing ECK-Tunnel on %s failed: %w", name, err)
	}
	return string(out), nil
}

// Upgrade reinstalls ECK-Tunnel on a server, which is how a node is brought to
