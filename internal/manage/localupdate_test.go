...
func TestOnlyThisMachinesArchiveIsOffered(t *testing.T) {
	...
	for _, name := range []string{
		"eck.tar.gz",
		"eck_linux_" + other + ".tar.gz",
		"eck_linux_" + runtime.GOARCH + ".tgz",
		"Eck_linux_" + runtime.GOARCH + ".tar.gz",
	} {
...
func TestAChecksumListCoveringAnotherArchiveIsKept(t *testing.T) {
	dir := t.TempDir()
	other := "eck_linux_arm64.tar.gz"
	if runtime.GOARCH == "arm64" {
		other = "eck_linux_amd64.tar.gz"
	}
...
func TestTheOperatorIsToldWhereToPutTheFile(t *testing.T) {
	...
	if !strings.HasPrefix(LocalAssetName(), "eck_linux_") ||
