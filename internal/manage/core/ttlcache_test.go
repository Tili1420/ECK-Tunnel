...
// The windows the journal cache uses, named here so the generic cache's tests
// exercise it at the cadence it is actually used at. The values themselves
// belong to the caller (internal/manage/logscache.go); what is under test is
// the cache.
const (
	logsCacheTTL   = 2 * time.Second
	logsCachePrune = 5 * time.Minute
)
...
func TestUnitStateIsSharedBetweenPollers(t *testing.T) {
	...
	for i := 0; i < 30; i++ {
		if !c.Get("is-active\x00eck-iran", ask) {
