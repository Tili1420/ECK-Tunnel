...
import (
	...
	"github.com/eck-tunnel/eck/internal/utils/network"
)
...
func systemChecks() []Check {
	...
	out = append(out, Check{Group: g, Name: "ECK-Tunnel version", Level: CheckInfo, Detail: app.Version})
	...
	if fi, err := os.Stat(app.BinPath); err == nil {
		...
		out = append(out, Check{Group: g, Name: "Binary", Level: CheckFail,
			Detail: "missing at " + app.BinPath, Fix: "reinstall ECK-Tunnel"})
	}
	...
	if os.Geteuid() == 0 {
		...
		out = append(out, Check{Group: g, Name: "Root privileges", Level: CheckFail,
			Detail: "not root", Fix: "run: sudo eck"})
	}
	...
	if _, err := exec.LookPath("systemctl"); err == nil {
		...
		out = append(out, Check{Group: g, Name: "systemd", Level: CheckFail,
			Detail: "systemctl not found", Fix: "ECK-Tunnel needs systemd to manage services"})
	}

	if runtime.GOOS != "linux" {
		out = append(out, Check{Group: g, Name: "Platform", Level: CheckWarn,
			Detail: runtime.GOOS, Fix: "ECK-Tunnel is designed for Linux servers"})
		return out
	}
...
func monitorChecks() []Check {
	...
	if !fileExists(app.ServiceDir + "/" + app.MonitorService) {
		return append(out, Check{Group: g, Name: "Service", Level: CheckWarn,
			...
			Fix:    "restart the CLI (sudo eck); it installs the service on launch"})
	}
	if MonitorRunning() {
...
func tunnelChecksFor(t Tunnel, pairs [][2]string) []Check {
	...
	case len(spec.Token) < 16 || spec.Token == "eck":
...
// Location is one file or directory ECK-Tunnel owns, and whether it exists.
...
// Locations lists every file and folder ECK-Tunnel uses, so a user can see at a
