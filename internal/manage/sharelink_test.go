...
func TestATruncatedLinkIsRefused(t *testing.T) {
	...
	for _, cut := range []int{len(s) - 1, len(s) - 5, len(s) / 2, len("eck://1.") + 4} {
...
func TestBadLinksSayWhatIsWrong(t *testing.T) {
	for _, tc := range []struct{ name, in, want string }{
		...
		{"no version separator", "eck://abcdef", "incomplete"},
		{"a version this build does not know", "eck://9.abcdef", "version 9"},
		{"damaged payload", "eck://1.!!!!not-base64!!!!", "damaged"},
	} {
