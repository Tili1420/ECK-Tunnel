...
// Release-based updater. ECK-Tunnel updates itself from GitHub release assets
// (eck_linux_amd64.tar.gz / eck_linux_arm64.tar.gz). Every network
...
import (
	...
	"encoding/hex"
	"fmt"
	"github.com/eck-tunnel/eck/internal/manage/backup"
	"io"
	"net/http"
	"os"
	"path/filepath"
...
func relayHTTPClient(timeout time.Duration) *http.Client {
	...
	for _, path := range matches {
		...
		if port := relayExposedPort(cfg.Server.Ports, cfg.Server.Token); port != "" {
			return socks.HTTPClient("127.0.0.1:"+port, "eck", cfg.Server.Token, timeout)
		}
	}
	return nil
}
...
func latestTag() (string, error) {
	...
	//    Last of all the self-hosted mirror's VERSION, for when GitHub is out
	//    of reach entirely; see mirror.go.
	for _, s := range releaseTries(rawURL, "VERSION", 20*time.Second) {
	...
	return "", fmt.Errorf("could not reach GitHub (direct or through the tunnel relay) or an update mirror: %v", lastErr)
}
...
// the same regardless of how eck was installed (release or git clone) —
...
func downloadAsset(tag, destDir string, logf func(string)) (string, error) {
	...
	var lastErr error = fmt.Errorf("no source reachable")
	for _, s := range releaseTries(url, tag+"/"+asset, 3*time.Minute) {
...
func fetchChecksums(tag, asset string) (sums []byte, want string, err error) {
	...
	var lastErr error = fmt.Errorf("no source reachable")
	// A 404 means "this release publishes no checksums" only when every
	// source that answered says so. A tag published to the mirror alone is a
	// 404 on GitHub, and stopping there would refuse an update the mirror
	// could have verified.
	notFound, answered := 0, 0
	for _, s := range releaseTries(url, tag+"/SHA256SUMS", 30*time.Second) {
		resp, err := s.client.Get(s.url)
		if err != nil {
			lastErr = err
			continue
		...
		if resp.StatusCode == http.StatusNotFound {
			resp.Body.Close()
			notFound++
			lastErr = fmt.Errorf("%s has no SHA256SUMS for %s", s.name, tag)
			continue
		}
		if resp.StatusCode != http.StatusOK {
			resp.Body.Close()
	...
	if answered > 0 && notFound == answered {
...
// extractBinary pulls the `eck` executable out of the release archive and
...
// extractBinaryTo pulls the `eck` binary out of a release archive and
// writes it to dest.
//
...
func extractBinaryTo(archive, dest string) error {
	...
	for {
		...
		if hdr.Typeflag != tar.TypeReg || filepath.Base(hdr.Name) != "eck" {
			continue
		}
	...
	return fmt.Errorf("no `eck` binary found inside the archive")
}
...
func ApplyUpdate(logf func(string)) error {
	...
	//
	// Nothing else rewrites /etc/sysctl.d/99-eck.conf, so a value this
