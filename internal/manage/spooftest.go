...
import (
	...
	"github.com/eck-tunnel/eck/internal/spooftest"
...
func spoofTestReceiver() {
	token := tui.PromptDefault("Shared token (must match the sender)", "eck")
...
func spoofTestSender() {
	...
	token := tui.PromptDefault("Shared token (must match the receiver)", "eck")
