...
func TestFailedBackupLeavesNothingBehind(t *testing.T) {
	...
	archives, _ := filepath.Glob(filepath.Join(dir, "eck-backup-*.tar.gz"))
	if len(archives) != 0 {
...
func TestPruneKeepsTheNewestAndSweepsStalePartials(t *testing.T) {
	...
	for i := 0; i < backupRetention+4; i++ {
		name := filepath.Join(dir, "eck-backup-2026010"+string(rune('0'+i%10))+"-000000.tar.gz")
		if err := os.WriteFile(name, []byte("x"), 0600); err != nil {
			t.Fatal(err)
		}
	}

	fresh := filepath.Join(dir, ".eck-backup-fresh.partial")
	stale := filepath.Join(dir, ".eck-backup-stale.partial")
...
func partials(t *testing.T, dir string) []string {
	t.Helper()
	matches, err := filepath.Glob(filepath.Join(dir, ".eck-backup-*.partial"))
	if err != nil {
		t.Fatal(err)
	}
	return matches
