...
func updateMenu() {
	for {
		...
		idx := tui.ChooseOpt("Choose:", []tui.Option{
			...
			{Title: "Update mirror", Desc: mirrorDesc()},
		})
		switch idx {
		case 0:
			runUpdate()
...
func runUpdate() {
	...
	tui.Info("ECK-Tunnel puts the previous version back automatically.")
	fmt.Println()
	...
	if err != nil {
		...
		tui.Success("ECK-Tunnel updated successfully.")
	}
	tui.PressEnter()
}
...
// mirrorDesc shows the configured update mirror on the menu line.
func mirrorDesc() string {
	if m := manage.UpdateMirror(); m != "" {
		return "your own download server when GitHub is unreachable — " + m
	}
	return "your own download server when GitHub is unreachable — not set"
}

// mirrorMenu sets or clears the self-hosted release mirror. Everything fetched
// from it is still checked against the signed checksum list, so the mirror
// itself does not need to be trusted.
func mirrorMenu() {
	tui.Clear()
	tui.Title("Update mirror")
	fmt.Println()
	tui.Info("A folder on any web server that holds the files scripts/make-release.sh")
	tui.Info("produces. Updates try GitHub first and use this when GitHub is unreachable.")
	tui.Info("Every file is still verified against the signed checksum list.")
	fmt.Println()
	cur := manage.UpdateMirror()
	if cur != "" {
		tui.Info("Current: " + cur)
	}
	v := tui.PromptDefault("Mirror URL (e.g. https://dl.example.com/eck, '-' to remove)", cur)
	if v == "-" {
		v = ""
	}
	if err := manage.SetUpdateMirror(v); err != nil {
		tui.Error(err.Error())
	} else if v == "" {
		tui.Success("Update mirror removed.")
	} else {
		tui.Success("Update mirror set.")
	}
	tui.PressEnter()
}
