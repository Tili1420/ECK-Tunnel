...
func configureAlerts(cfg telegram.Config) {
	...
	if a.Enabled {
		...
		a.NewRelease = tui.Confirm("Tell me when a new ECK-Tunnel version is released", a.NewRelease)
	...
	tui.Info(a.Summary())
	fmt.Println()
	tui.Warn("Watched by the eck-monitor service, which runs on its own —")
...
// the primary path relays traffic through a tunnel: eck forwards a
