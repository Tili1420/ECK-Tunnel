// Package menu implements the interactive eck CLI shown when the binary
...
func printMenu() {
	...
	menuItem(6, "Optimize", "kernel tuning, best DNS, best apt mirror")
...
func requireRoot() {
	if os.Geteuid() != 0 {
		tui.Error("ECK-Tunnel must be run as root (use: sudo eck).")
		os.Exit(1)
	}
}
