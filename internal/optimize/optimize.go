...
var sysctlFile = "/etc/sysctl.d/99-eck.conf"

const limitsFile = "/etc/security/limits.d/99-eck.conf"

const limitsContent = `# Raised by eck for high connection counts
* soft nofile 1048576
* hard nofile 1048576
...
func Apply(logf func(string), reserve []int) {
	...
	b.WriteString("# Managed by eck — network optimizations\n")
	for _, kv := range rows {
		fmt.Fprintf(&b, "%s = %s\n", kv[0], kv[1])
	}
