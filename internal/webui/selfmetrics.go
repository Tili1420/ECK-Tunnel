...
func writePanelMetrics(b *strings.Builder) {
	...
	gauge(b, "eck_panel_uptime_seconds", "How long this panel process has been running",
		time.Since(p.started).Seconds())

	counterHead(b, "eck_panel_requests_total", "Requests served, by outcome")
	fmt.Fprintf(b, "eck_panel_requests_total{outcome=\"ok\"} %d\n", p.ok.Load())
	fmt.Fprintf(b, "eck_panel_requests_total{outcome=\"refused\"} %d\n", p.refused.Load())
	fmt.Fprintf(b, "eck_panel_requests_total{outcome=\"failed\"} %d\n", p.failed.Load())
	...
	gauge(b, "eck_panel_request_ms_mean", "Mean time to serve a request, milliseconds", mean)
	gauge(b, "eck_panel_request_ms_max", "Slowest request since this panel started, milliseconds",
		float64(p.maxNanos.Load())/1e6)

	jobsMu.Lock()
	done, failed := jobsDone, jobsFailed
	jobsMu.Unlock()
	counterHead(b, "eck_panel_jobs_total", "Background jobs this panel has run, by outcome")
	fmt.Fprintf(b, "eck_panel_jobs_total{outcome=\"done\"} %d\n", done)
	fmt.Fprintf(b, "eck_panel_jobs_total{outcome=\"failed\"} %d\n", failed)
}
