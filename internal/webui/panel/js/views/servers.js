 ...
 *
 * CLI: nothing. There is no ECK-Tunnel state on a managed server to configure.
 */
...
export function serversView(ctx) {
  ...
  function nodeCard(n) {
    ...
    /* One line of identity under the name: the address, where it is, which
       ECK-Tunnel it runs, how long it has been up. They were four separate
  ...
  form.addEventListener('submit', async ev => {
    ...
    /* Adding can take minutes rather than seconds, because a server with no
       ECK-Tunnel on it gets one. Said plainly while it happens: a button that sits
       there for two minutes with no explanation is one people press again. */
    note.textContent = 'Logging in, and installing ECK-Tunnel if that server has none. '
