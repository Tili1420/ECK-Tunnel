...
import (
	...
	"github.com/eck-tunnel/eck/internal/node"
)
...
func TestTheFarSideNeedsNothingButTheOneCommand(t *testing.T) {
	...
	if !strings.Contains(src, `case "exec":`) {
		t.Fatal("`eck node exec` is gone, and it is the only thing the panel runs " +
			"on a managed server")
	}
	...
	if strings.Contains(string(sh), "node setup") {
		t.Error("install.sh still ends in `eck node setup`, which no longer exists")
	}
	// And it must still install without a terminal, because that is how the
	// panel runs it on a server that has no ECK-Tunnel yet.
...
func (r *installingRunner) Call(name, op string, body, out any) error {
	...
	if !r.up[name] {
		return node.ErrOffline{
			Name: name,
			Why:  "the ECK-Tunnel on that server is too old to be managed from this panel",
...
// A server already running an older ECK-Tunnel is upgraded, not refused.
//
// This is the bug report: the panel reached a server in the operator's fleet,
// found a ECK-Tunnel that did not understand `node exec`, and treated it as a
// server that could not be reached — refusing the add and printing the far
// machine's own help, which told them to run `eck node setup`, a command
...
func TestAServerRunningAnOlderEckIsUpgradedNotRefused(t *testing.T) {
	isolateFleet(t)
	s := newFleetServer()
	t.Cleanup(s.nodes.Stop)
	...
	if w.Code != http.StatusOK {
		t.Fatalf("a server running an older ECK-Tunnel was refused: %d %s", w.Code, w.Body.String())
	}
	if r.installed != 1 {
