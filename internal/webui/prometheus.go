...
import (
	...
	"fmt"
	"github.com/eck-tunnel/eck/internal/tunhist"
	"net/http"
	"strconv"
	"strings"
	...
	"github.com/eck-tunnel/eck/internal/metrics"
	"github.com/eck-tunnel/eck/internal/sysstat"
)
...
func (s *server) handlePrometheus(w http.ResponseWriter, r *http.Request) {
	...
	m := sysstat.Get()
	gauge(&b, "eck_cpu_percent", "CPU usage percent", m.CPUPercent)
	gauge(&b, "eck_mem_percent", "Memory usage percent", m.MemPercent)
	gauge(&b, "eck_swap_percent", "Swap usage percent", m.SwapPercent)
	gauge(&b, "eck_disk_percent", "Disk usage percent", m.DiskPercent)
	gauge(&b, "eck_uptime_seconds", "System uptime in seconds", m.Uptime.Seconds())
	gauge(&b, "eck_monitor_running", "1 when the eck-monitor service is active",
		boolVal(manage.MonitorRunning()))

	tunnels := manage.List()
	health := manage.AllHealth()

	b.WriteString("# HELP eck_tunnel_up 1 when the tunnel's peer is connected\n# TYPE eck_tunnel_up gauge\n")
	for _, t := range tunnels {
		up := health[t.Name].State == "online"
		fmt.Fprintf(&b, "eck_tunnel_up{name=%q,transport=%q,role=%q} %d\n",
			t.Name, t.Transport, t.Role, int(boolVal(up)))
	}

	counterHead(&b, "eck_tunnel_bytes_in_total", "Bytes received over the tunnel")
	counterHead(&b, "eck_tunnel_bytes_out_total", "Bytes sent over the tunnel")
	...
	for _, t := range tunnels {
		...
		fmt.Fprintf(&b, "eck_tunnel_bytes_in_total{name=%q} %d\n", t.Name, snap.BytesIn)
		fmt.Fprintf(&b, "eck_tunnel_bytes_out_total{name=%q} %d\n", t.Name, snap.BytesOut)
		if snap.Runtime != nil {
	...
	// somebody will page on.
	{
		...
		if any {
			gaugeHead(&b, "eck_tunnel_uptime_percent",
				"Percentage of health checks in the last 7 days that saw the tunnel up")
			gaugeHead(&b, "eck_tunnel_uptime_checks",
			...
			for _, t := range tunnels {
				...
				fmt.Fprintf(&b, "eck_tunnel_uptime_percent{name=%q} %.4f\n", t.Name, pct)
				fmt.Fprintf(&b, "eck_tunnel_uptime_checks{name=%q} %d\n", t.Name, checks)
			}
		}
	}

	if len(runtimeSnaps) > 0 {
		gaugeHead(&b, "eck_tunnel_goroutines", "Goroutines held by the tunnel process")
		gaugeHead(&b, "eck_tunnel_open_files", "File descriptors held by the tunnel process")
		gaugeHead(&b, "eck_tunnel_heap_bytes", "Heap bytes in use by the tunnel process")
		gaugeHead(&b, "eck_tunnel_heap_objects", "Live heap objects in the tunnel process")
		for _, r := range runtimeSnaps {
			fmt.Fprintf(&b, "eck_tunnel_goroutines{name=%q} %d\n", r.name, r.rs.Goroutines)
			if r.rs.OpenFiles > 0 {
				fmt.Fprintf(&b, "eck_tunnel_open_files{name=%q} %d\n", r.name, r.rs.OpenFiles)
			}
			fmt.Fprintf(&b, "eck_tunnel_heap_bytes{name=%q} %d\n", r.name, r.rs.HeapBytes)
			fmt.Fprintf(&b, "eck_tunnel_heap_objects{name=%q} %d\n", r.name, r.rs.HeapObjects)
		}
	}
	...
	if len(kcpSnaps) > 0 {
		for _, c := range []struct {
			...
			{"eck_kcp_retransmitted_total", "KCP segments sent again", func(k *metrics.KCPStats) uint64 { return k.Retransmitted }},
			{"eck_kcp_lost_total", "KCP segments that never arrived", func(k *metrics.KCPStats) uint64 { return k.Lost }},
			{"eck_kcp_fec_recovered_total", "Packets rebuilt by forward error correction", func(k *metrics.KCPStats) uint64 { return k.FECRecovered }},
		} {
