...
// until now it was: every ECK-Tunnel server on earth returned byte-identical
...
// finds every ECK-Tunnel server there is, no token and no probing required. A
...
func newDecoyProfile(token string) decoyProfile {
	...
	h := sha256.Sum256([]byte("eck-decoy\x00" + token))
