...
Cosmetic. It names the systemd service (`eck-<name>`) and the config file.
Press Enter.
...
```
sudo eck  →  3. Manage  →  Status          # live table, both ends
sudo eck  →  3. Manage  →  Health Check    # finds problems, prints the fix
```
...
**روی سرور ایران:** `sudo eck` → گزینهٔ ۱ (Setup Iran) → Reverse → خانوادهٔ TCP →
...
**روی سرور خارج:** `sudo eck` → گزینهٔ ۲ (Setup Kharej) → Reverse → همان ترنسپورت →
