...
> `sudo eck` → Setup Iran / Setup Kharej → **Direct**, then choose **Spoof**
...
On both machines: `sudo eck` → Setup Iran / Setup Kharej → **Direct**, and
...
```
sudo eck  →  3. Manage  →  IP Spoofing Tester
```
...
```
sudo eck  →  3. Manage  →  Manage Tunnels  →  <tunnel>  →  Edit  →  IP Spoofing
```
