...
```
sudo eck  →  3. Manage  →  Manage Tunnels  →  <tunnel>  →  Edit
                →  Forward UDP
```
...
In `/etc/eck/<name>.toml`, under `[server]`:
...
```bash
ss -lnup | grep :443      # UDP  — should show eck
ss -lntp | grep :443      # TCP
```

If the UDP line is missing, the setting is not on. If the UDP side of a port
cannot be bound because something else already holds it, ECK-Tunnel warns and
...
`/etc/eck/<name>.toml` زیر `[server]` بنویس `accept_udp = true` و ری‌استارت
