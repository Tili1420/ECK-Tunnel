...
The most common ECK-Tunnel deployment: a VPN panel on the kharej server, users
...
# on Iran — ECK-Tunnel holds the exposed port
ss -tlnp | grep :443
ss -lnup  | grep :443        # only if UDP forwarding is on

sudo eck → Manage → Health Check
```
...
را روشن کن، **بعد** در ECK-Tunnel `Manage → Edit → Real client IP`. برعکسش همهٔ
