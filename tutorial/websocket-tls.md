...
- **A real Chrome TLS fingerprint.** Go's default ClientHello is itself
  identifiable; ECK-Tunnel sends a current Chrome one, so the handshake blends into
...
A self-signed certificate **encrypts exactly as well** — the client is ECK-Tunnel's
...
If it does not appear: `journalctl -u eck-<name> -n 50`.
