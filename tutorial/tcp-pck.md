...
A TCP transport that **does not use the kernel's TCP stack**. ECK-Tunnel builds the
...
the connection ending. ECK-Tunnel installs two narrow rules on start and removes
them on stop:
...
Tagged `eck-pck-<port>`, so one left behind by a crash is easy to find:

```bash
iptables-save | grep eck-pck
```
...
هنگام استارت دو قانون iptables با تگ `eck-pck-<port>` اضافه می‌کند و موقع
