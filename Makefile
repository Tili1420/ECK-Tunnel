...
release-linux:
	mkdir -p dist
	@for a in $(ARCHES); do 	  echo "  building linux/$$a"; 	  CGO_ENABLED=0 GOOS=linux GOARCH=$$a 	    go build -trimpath -ldflags "$(LDFLAGS)" -o dist/eck-linux-$$a . || exit 1; 	done
	@for v in $(ARMS); do 	  echo "  building linux/armv$$v"; 	  CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=$$v 	    go build -trimpath -ldflags "$(LDFLAGS) -X github.com/eck-tunnel/eck/internal/app.GOARM=$$v" 	    -o dist/eck-linux-armv$$v . || exit 1; 	done

# GitHub release assets: eck_linux_<arch>.tar.gz, each containing a single
# `eck` binary. These are what install.sh and the in-app updater download.
release: version release-linux
	mkdir -p release
	@for a in $(ARCHES) $(addprefix armv,$(ARMS)); do 	  cp dist/eck-linux-$$a dist/eck && 	  tar -czf release/eck_linux_$$a.tar.gz -C dist eck && 	  rm dist/eck || exit 1; 	done
	...
	cd release && (sha256sum eck_linux_*.tar.gz > SHA256SUMS 2>/dev/null || shasum -a 256 eck_linux_*.tar.gz > SHA256SUMS)
...
sbom:
	@mkdir -p release
	@{ 	  echo "# ECK-Tunnel $$(cat VERSION) — bill of materials"; 	  echo "#"; 	  echo "# Read out of the built binary, so it describes what was linked"; 	  echo "# rather than what go.mod asked for."; 	  echo "#"; 	  echo "# Verify a binary you downloaded against this with:"; 	  echo "#     go version -m ./eck"; 	  echo; 	  go version -m dist/eck-linux-amd64; 	} > release/SBOM.txt
...
reproducible:
	@CGO_ENABLED=0 go build -trimpath -ldflags "$(LDFLAGS)" -o /tmp/eck-repro-a .
	@CGO_ENABLED=0 go build -trimpath -ldflags "$(LDFLAGS)" -o /tmp/eck-repro-b .
	@if cmp -s /tmp/eck-repro-a /tmp/eck-repro-b; then 	  echo "reproducible: two builds are byte-identical"; 	else 	  echo "NOT reproducible: two builds of the same source differ"; exit 1; 	fi
	@rm -f /tmp/eck-repro-a /tmp/eck-repro-b
