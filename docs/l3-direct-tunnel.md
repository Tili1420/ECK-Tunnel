...
Every other transport in ECK-Tunnel forwards **ports**: a listener on the Iran
...
It is the GRE/IPIP idea, built into ECK-Tunnel's own core rather than borrowed
from the kernel, and carried inside ECK-Tunnel's own transports.
...
ECK-Tunnel writes the same GRE header — RFC 2784, with the RFC 2890 key — but the
...
- **It does not interoperate.** A Cisco, a MikroTik or a plain Linux GRE
  endpoint cannot talk to it. ECK-Tunnel talks to ECK-Tunnel.
...
**From the menu — the easy way.** Run `sudo eck`, choose **Setup Iran** or
...
|---|---|---|
| `mode` | *required* | `dial` or `listen`. Its absence is what tells ECK-Tunnel there is no layer-3 tunnel here at all |
...
- It is for the **`udp` carrier only**. The obfuscated carriers already vary
  ...
  ECK-Tunnel refuses `paths` on them rather than writing a setting that does
  nothing.
...
> reply from the packets themselves and must be told. ECK-Tunnel refuses the
...
GRE and IPIP have no encryption of their own; all of the above is ECK-Tunnel's,
...
ECK-Tunnel rewrites the MSS option in the SYN of each TCP connection leaving the
...
```bash
iptables -t mangle -S | grep eck-l3-mss
```
...
هر ترنسپورت دیگری در ECK-Tunnel **پورت** forward می‌کند: یک listener روی ایران، یک
...
می‌تواند رویش route شود. همان ایدهٔ GRE/IPIP است، ولی داخل هستهٔ خود ECK-Tunnel و
...
ECK-Tunnel همان هدر GRE را می‌نویسد، ولی آن هدر چیزی نیست که سفر می‌کند: داخل یک
...
سیسکو، MikroTik یا GRE لینوکسی نمی‌توانند با آن حرف بزنند. ECK-Tunnel با ECK-Tunnel.
