...
Everything here can be done from `sudo eck` → **Health Check** and
...
ECK-Tunnel now notices this on its own: the watchdog watches for **one direction
...
```
sudo eck        # → Diagnose → the tunnel
```

Look for the path MTU line. If it is below 1500, clamp to it:

```
sudo eck        # → Manage tunnels → <tunnel> → TCP MSS clamp
```
...
ECK-Tunnel reports this itself when it can see it. The panel's tunnel card shows a
"last hop" warning, and so does the terminal:

```
eck tunnel status <name>
...
...
```
sudo cat /etc/eck/<name>.toml          # on this machine
```
...
and several real faults present this way. ECK-Tunnel reports repeated restarts as
...
```
systemctl status eck-webui
journalctl -u eck-webui -n 50
```
...
ECK-Tunnel takes a snapshot before every update and rolls back on its own if the
tunnels do not come back. If you are reading this, that either worked or you
want to do it by hand:

```
sudo eck        # → Update → Rollback
```
...
```
journalctl -u eck-<name> -f       # one tunnel, live
journalctl -u eck-monitor -n 100   # the watchdog, the bot, the alerts
journalctl -u eck-webui -n 100     # the panel
```
...
```
sudo sed -i 's/^log_level = .*/log_level = "debug"/' /etc/eck/<name>.toml
sudo systemctl restart eck-<name>
journalctl -u eck-<name> -f
```
...
eck version
eck tunnel list
sudo eck        # → Diagnose, and copy the output
journalctl -u eck-<name> -n 200
```
...
`sudo eck` → **Health Check** و **Diagnose** هم در دسترس است.

### تونل می‌گوید بالاست ولی چیزی رد نمی‌شود

گران‌ترین خرابی این محصول، و فهرست علت‌هایش کوتاه است. خود ECK-Tunnel این را
...
سرور **خارج**: `ss -tlnp | grep <port>`. خود ECK-Tunnel هم وقتی ببیند می‌گوید —
...
۴. **ببین واقعاً چه حمل می‌کند:** `eck tunnel status <name>`. بایت ورودی و
...
`eck check -c /etc/eck/<name>.toml`، بعد به همین ترتیب: توکن یکی باشد
...
اول `sudo eck → Alerts` — ری‌استارت‌های پیاپی به‌صورت **یک** وضعیت گزارش
...
`systemctl status eck-webui` و `journalctl -u eck-webui -n 50`. پنل زیر
یک **مسیر مخفی** جواب می‌دهد و جای دیگری نه، پس bookmark بدون آن ۴۰۴ می‌گیرد نه
صفحهٔ ورود. مسیر در `sudo eck → Web Panel` نوشته شده.
...
دستی: `sudo eck → Update → Rollback`. اگر تونل‌ها برگشتند ولی رفتار فرق
دارد، نسخهٔ دو طرف را با `eck version` مقایسه کن — نسخهٔ جدید با قدیم کار
...
```
journalctl -u eck-<name> -f
journalctl -u eck-monitor -n 100
journalctl -u eck-webui -n 100
```
...
`eck version`، `eck tunnel list`، خروجی `Diagnose`، و
`journalctl -u eck-<name> -n 200`. تونل دو سر دارد و تقریباً هر سؤالی دربارهٔ
