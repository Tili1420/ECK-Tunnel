...
> ECK-Tunnel's own GRE — because that shape covers the same job and measures its
...
The same forwarded ports ECK-Tunnel has always served, with the tunnel dialled
the other way round.
...
**From the menu — the easy way.** Run `sudo eck` and choose **Setup Iran** or
...
|---|---|---|
| `role` | *required* | `iran` or `kharej`. Its absence is what tells ECK-Tunnel there is no direct tunnel here |
