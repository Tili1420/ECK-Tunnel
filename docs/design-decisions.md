...
What ECK-Tunnel deliberately does not do, and why.
...
**The data path carries opaque bytes.** ECK-Tunnel does not look at what it
...
ECK-Tunnel is AGPL-3.0, and `NOTICE` records that part of the data plane derives
...
ECK-Tunnel is one hop carrying opaque bytes — there is no span structure to
...
Provisioning ECK-Tunnel from existing infrastructure-as-code.
...
who wants it, without a line in this repo. What ECK-Tunnel should do instead is
...
Running ECK-Tunnel in a container.
...
**Rejected.** ECK-Tunnel carries opaque bytes for a proxy that sits behind it; it
...
mean either a user database ECK-Tunnel cannot populate or traffic inspection that
...
ECK-Tunnel in the position of inspecting traffic. The l3 engine carries whatever
...
ECK-Tunnel sits at the edge, carrying traffic that arrives already anonymous — it
has neither the DNS view nor the process view. This is Xray/sing-box's job, and
those already sit behind a ECK-Tunnel tunnel in the normal deployment. Adding it
...
این صفحه فهرست کارهایی است که ECK-Tunnel **عمداً نمی‌کند**، و دلیل هر کدام.
...
**مجوز، که تعیین می‌کند چه چیزی قابل فروش است:** ECK-Tunnel با AGPL-3.0 منتشر شده
