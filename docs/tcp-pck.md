...
ECK-Tunnel installs two narrow rules on start and removes them on stop:
...
They are tagged `eck-pck-<port>`, so one left behind by a crash is easy to
find:
...
می‌دهد و آن RST برای هر دستگاه stateful وسط راه یعنی «اتصال تمام شد». ECK-Tunnel موقع
استارت دو قانون باریک با تگ `eck-pck-<port>` اضافه و موقع استاپ حذف می‌کند.
