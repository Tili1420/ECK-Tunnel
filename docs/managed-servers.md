...
The panel dials out. The far server opens nothing for ECK-Tunnel, and the panel
...
If ECK-Tunnel is not installed there, the panel installs it — that is the ordinary
...
In `/etc/eck/nodes.json`, on the panel's own server, `0600` and owned by
root — the same file and the same permissions as every other secret ECK-Tunnel
...
Nothing. There is no ECK-Tunnel service, no config and no state that belongs to
...
The one command is `eck node exec`, which performs a single operation from
...
**جهت اتصال:** پنل زنگ می‌زند. سرور دور چیزی برای ECK-Tunnel باز نمی‌کند و پنل هم
...
**رمز کجاست:** در `/etc/eck/nodes.json` روی سرور خود پنل، با مجوز `0600` و
مالکیت root — همان فایل و همان مجوزی که هر راز دیگر ECK-Tunnel دارد. هیچ‌وقت به
