...
| 9 | **Uninstall** | Removes everything ECK-Tunnel installed. |
| 10 | **Exit** | |
...
| **Tunnel name** | names the service (`eck-<name>`) and the config file |
...
A direct tunnel is always a full IP tunnel wrapped in ECK-Tunnel's own GRE, so
...
| **Game Latency Test** | Estimates the in-game ping a player would feel through this exit — pings the nearest edge of Dota 2, CS2, Valorant, PUBG, Fortnite and others from the kharej server, adds the tunnel leg, and rates the result. Endpoint list at `/etc/eck/game-endpoints.list`. |
...
همه‌چیز از یک منو در دسترس است: `sudo eck`.
