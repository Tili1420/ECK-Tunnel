...
ECK-Tunnel logs to stdout, systemd puts that in journald, and on one server
`journalctl -u eck-<name>` is the whole story. With five servers it is five
...
ECK-Tunnel does not ship logs itself, and
...
common collectors — both read journald, so neither needs ECK-Tunnel to write a
file.
...
[sources.eck]
type = "journald"
include_units = ["eck-fr-relay", "eck-de-relay"]
...
```
{job="eck", level="error"}                  every error in the fleet
{job="eck", tunnel="fr-relay"} |= "restart" one tunnel's restarts
{job="eck", transport="quic", level="warn"} one carrier, everywhere
```

## What not to expect

- **ECK-Tunnel does not retry or buffer.** It writes to stdout; journald and the
...
- **There is no trace context.** ECK-Tunnel is one hop carrying opaque bytes, so
...
ECK-Tunnel روی stdout لاگ می‌نویسد، systemd آن را در journald می‌گذارد، و روی یک
سرور `journalctl -u eck-<name>` همهٔ داستان است. با پنج سرور می‌شود پنج
...
**فرستادنش:** ECK-Tunnel خودش لاگ نمی‌فرستد و
...
**چیزی که انتظارش را نداشته باش:** ECK-Tunnel بافر یا retry نمی‌کند — بعد از stdout
