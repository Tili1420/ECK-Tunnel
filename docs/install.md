...
into `/root/ECK-Tunnel`, **verifies it against the checksum published with the
...
Everything lands in a tidy layout — the release bundle in `/root/ECK-Tunnel`,
backups in `/root/ECK-Tunnel/backups`, tunnel configs in `/etc/eck`. See
...
`x86_64` → `eck_linux_amd64.tar.gz`, `aarch64` → `eck_linux_arm64.tar.gz`.
...
```bash
scp install.sh SHA256SUMS eck_linux_amd64.tar.gz root@SERVER_IP:/root/
...
```bash
sha256sum eck_linux_amd64.tar.gz        # compare against SHA256SUMS
tar xzf eck_linux_amd64.tar.gz
mkdir -p /etc/eck /root/ECK-Tunnel/backups
install -m 0755 eck /usr/local/bin/eck
echo /root/ECK-Tunnel > /etc/eck/install_path
sudo eck
```
...
binary in place, and your tunnels in `/etc/eck` are untouched. Restart them
afterwards with `sudo eck` → **Manage → Restart ALL**.
...
**Main menu → 9) Uninstall** removes everything ECK-Tunnel installed.
...
می‌کند. بعداً با `sudo eck` منو را باز کن.
