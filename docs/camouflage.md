...
Looking ordinary is only half of it. If every ECK-Tunnel install answered with the
same bytes, a single internet-wide scan for that exact response would find every
ECK-Tunnel server there is — no token and no probing needed. Camouflage everyone
...
**هر سرور، یک سرور متفاوت:** اگر همهٔ نصب‌های ECK-Tunnel بایت‌به‌بایت یکسان جواب
می‌دادند، یک اسکن سراسری روی همان پاسخ، تمام سرورهای ECK-Tunnel دنیا را پیدا می‌کرد —
