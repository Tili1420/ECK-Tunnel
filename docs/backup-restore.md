...
Backups live in `/root/ECK-Tunnel/backups`.
...
`sudo eck` → **Backup & Restore** → **Show the fleet key**
...
`sudo eck` → **Backup & Restore** → **Restore the fleet key**
...
`/root/ECK-Tunnel/backups` ذخیره می‌شوند.
...
Backups are written to `/var/backups/eck` — on the server they describe.
...
```
rclone copy {} remote:eck/
scp {} backup@10.0.0.9:/srv/eck/
restic backup {}
```
