...
What ECK-Tunnel is made of, and which part answers which question. The topic
...
|---|---|
| `eck` | the interactive management menu — `internal/menu` |
| `eck -c <file>` | **engine mode**: one tunnel, from one config |
| `eck --webui` | the web panel — `internal/webui` |
| `eck --monitor` | watchdog, Telegram bot, alerts, history — `internal/monitor` |
| `eck --proxy` | the built-in SOCKS5/HTTP proxy — `internal/localproxy` |
| `eck --restart-all` / `--telegram-report` | one-shot jobs, run from cron |
| `eck node exec <base64>` | the one operation a panel runs on a managed server over SSH |
...
`eck -c /etc/eck/<name>.toml`. This is the single most valuable
...
```
  ...
  ──────────────                    ───────────────────
  internal/metrics  ──writes──►  /etc/eck/<name>.metrics.json
                                        │
                                        ├──►  the panel
                                        ├──►  the Telegram bot
                                        ├──►  the CLI
...
agent, no daemon and no state — `eck node exec <base64>` performs one
...
این صفحه می‌گوید ECK-Tunnel از چه ساخته شده و کدام بخش به کدام سؤال جواب می‌دهد.
بقیهٔ راهنماها می‌گویند «چطور کاری را بکنی»؛ این یکی می‌گوید «چیزها کجایند».

**یک باینری، هفت حالت.** `main.go` قبل از هر چیز دیگری تصمیم می‌گیرد: `eck`
منوی مدیریت است؛ `eck -c <file>` **حالت موتور** است یعنی یک تونل از یک
...
یک‌باره‌ای که از cron اجرا می‌شوند؛ و `eck node exec <base64>` تنها عملیاتی
