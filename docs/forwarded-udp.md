...
With it off, the forwarded port binds TCP only, which is what ECK-Tunnel did
before v1.7.1.
...
2. **Both ends upgraded?** `eck -v` on each. A mixed pair forwards TCP
   only.
...
4. **Is it turned off?** `grep accept_udp /etc/eck/<tunnel>.toml`. An
   explicit `false` is honoured.
5. **Check the log** — `journalctl -u eck-<tunnel> -n 50`. The line
...
می‌دهد؛ `grep accept_udp /etc/eck/<tunnel>.toml` را نگاه کن؛ و در لاگ دنبال
