#!/usr/bin/env bash
#
# ECK-Tunnel installer — one command on the VPS (as root):
#
...
# /root/ECK-Tunnel and installs the binary, verifying it against the checksum
...
# Later, reopen it any time with:  sudo eck
#
set -euo pipefail
...
REPO="${ECK_REPO:-Tili1420/ECK-Tunnel}"
# A self-hosted copy of the releases, used when GitHub cannot be reached:
#   ECK_MIRROR=https://dl.example.com/eck bash install.sh
MIRROR="${ECK_MIRROR:-}"
MIRROR="${MIRROR%/}"
BIN_PATH="/usr/local/bin/eck"
INSTALL_DIR="/root/ECK-Tunnel"
...
# installed ECK-Tunnel and then enrolled the machine with a panel. A panel reaches
...
if [[ $# -gt 0 ]]; then
  ...
  err "This script takes no arguments. Run it to install ECK-Tunnel."
...
ASSET="eck_linux_${ARCH}.tar.gz"
mkdir -p /etc/eck "$INSTALL_DIR/backups"
...
# verify_signature <sumsfile> <sigfile>
...
# there yet. An account on the box pre-creates eck_linux_<arch>.tar.gz and
...
install_release() {
  ...
  # 2) The latest GitHub release; failing that, the self-hosted mirror.
  local base="https://github.com/${REPO}/releases/latest/download" from_mirror=0
  if ! fetch "${base}/${ASSET}" "$INSTALL_DIR/$ASSET"; then
    [[ -n "$MIRROR" ]] || return 1
    local tag
    tag="$(curl -fsSL --connect-timeout 15 "${MIRROR}/VERSION" 2>/dev/null | tr -d '[:space:]' || true)"
    if [[ ! "$tag" =~ ^v?[0-9]+(\.[0-9]+){0,3}$ ]]; then
      err "The mirror ${MIRROR} has no valid VERSION file."
      return 1
    fi
    base="${MIRROR}/${tag}"
    from_mirror=1
    warn "GitHub is unreachable — installing ${tag} from the mirror."
    fetch "${base}/${ASSET}" "$INSTALL_DIR/$ASSET" || return 1
  fi
  ...
  if ! fetch "${base}/SHA256SUMS" "$INSTALL_DIR/SHA256SUMS"; then
  ...
  # The checksum list arrives through the same channel as the archive, so on
  # its own it proves only that the download is intact. The signature proves
  # the list is the publisher's. From the mirror it is required; from GitHub a
  # release published before signing has none, which is only warned about.
  local sigrc=0
  if fetch "${base}/SHA256SUMS.sig" "$INSTALL_DIR/SHA256SUMS.sig"; then
    verify_signature "$INSTALL_DIR/SHA256SUMS" "$INSTALL_DIR/SHA256SUMS.sig" || sigrc=$?
  else
    sigrc=3
  fi
  if [[ $sigrc -eq 0 ]]; then
    info "Release signature verified."
  elif [[ ( $sigrc -eq 3 || $sigrc -eq 2 ) && $from_mirror -eq 0 ]]; then
    warn "This release has no signature — relying on the checksum from GitHub."
  else
    err "The release signature is missing or does not match ECK-Tunnel's key."
    err "Refusing to install it."
    rm -f "$INSTALL_DIR/$ASSET"
    exit 1
  fi
...
install_binary_from_tar() {
  tar -xzf "$INSTALL_DIR/$ASSET" -C "$INSTALL_DIR" eck
  install -m 0755 "$INSTALL_DIR/eck" "$BIN_PATH"
  rm -f "$INSTALL_DIR/eck"
  echo "$INSTALL_DIR" > /etc/eck/install_path
  # Remembered so later updates can fall back to the same mirror.
  if [[ -n "$MIRROR" ]]; then echo "$MIRROR" > /etc/eck/update-mirror; fi
}
...
# Open the menu straight away — people miss the "now run sudo eck" step.
...
if [ -t 0 ]; then
  echo -e "Starting the menu... ${GRAY}(next time, just run ${NC}${RED}sudo eck${GRAY})${NC}"
  echo
  exec "$BIN_PATH"
else
  echo -e "Open the menu with:  ${RED}sudo eck${NC}"
fi
