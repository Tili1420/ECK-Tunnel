...
// `eck node ...` — the managed side of the panel-to-server channel.
//
...
const nodeUsage = `eck node — the panel-managed side of this server

  eck node exec <request>
        Perform one operation and print the answer. Both are JSON, base64
        encoded. This is what a ECK-Tunnel panel runs over SSH; there is no
        reason to type it.
